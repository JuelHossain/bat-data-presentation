...
export default function DataContainer({ active }) {
  ...
  return (
    <motion.div className="flex gap-6 " key={title}>
...
function ComparedData({ ratio }) {
  ...
  return (
    <motion.div ref={ref} className="flex flex-col gap-4 flex-1 basis-1/3 items-end justify-between ">
      <motion.div className="flex flex-col items-end">
        <div className="flex">
          <motion.h2 className="text-[180px] font-bold ">{data}</motion.h2>
          <motion.h2 className="text-[180px] font-bold ">%</motion.h2>
        </div>

        <motion.h2 className="text-2xl font-bold -mt-10 ml-40 text-end">
